...
<?php $d = new Page;  ?>
<?php $s_all = $data['stock_out_order']; 
$cus_name = $d->stock_out_details_forcust($s_all->stock_out_id);
?>
<!-- Start Breadcrumbbar -->
<div class="breadcrumbbar">
	<div class="row align-items-center">
		<div class="col-md-8 col-lg-8">
			<h4 class="page-title">Make Sales Invoice</h4>
...
<!-- End Breadcrumbbar -->
<!-- Start Contentbar -->
<form method="POST" action="<?php echo URLROOT; ?>/pages/makenew_sales_order" enctype="multipart/form-data" id="salesForm">
				...
				<div class="card m-b-30">
					...
					<div class="card-body">
						<div class="form-group">
							...
							<input type="text" name="customer_id" id="cname_id" style="display: none;" value="<?php echo $cus_name->customer_id;?>"/>

							<input type="text" class="form-control" placeholder="Enter Customer Name" name="customer" id="cname" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" autocomplete="off" name="customer" value="<?php echo $cus_name->customer;?>" readonly>
						...
						<div class="form-group">
							...
							<textarea class="form-control" id="bill_address" name="bill_address" ></textarea>
						</div>
						<div class="form-group">
							<label>Sales Order#</label>
							<input type="text" class="form-control" placeholder="ex S0-00001" name="sales_order">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<select name="state_for_tax" id="stateSelect" class="form-control">
								...
								<option selected value="1">Within State</option>
								<option value="2" >Outside State</option>
							</select>
						</div>
						<div class="form-group">
							<label>Sales Order Date</label>
							<input type="date" class="form-control" placeholder="" name="ndate" min='1960-01-01' max="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>">
						</div>
					</div>
				</div>
			</div>
				...
				<div class="card m-b-30">
					...
					<div class="card-body">
						...
						<div class="form-group">
							...
							<input type="date" class="form-control" placeholder="" name="expected_delivery_date" min='1960-01-01' max="<?php echo date('Y-m-d'); ?>" value="<?php echo date('Y-m-d'); ?>">
						</div>
						<div class="form-group">
								...
								<table class="table table-bordered table-hover" id="tab_logic">
									<thead>
										<tr>
											...
											<th class="text-center" width="150"> Tax(%) </th>
									...
									$item_id = explode("|||", $s_all->item_id);
									$item_name = explode("|||", $s_all->item_name);
									$item_qty = explode("|||", $s_all->item_qty);
									$item_rec = explode("|||", $s_all->item_rec);
									$item_price = explode("|||", $s_all->item_price);
									$item_rtotal = explode("|||", $s_all->item_rtotal);
									...
									<?php for ($i=0; $i < sizeof($item_id); $i++) { 
										...
										<tr>
											<td><?php echo $i+1;?></td>
											<td>
												<input type="text" placeholder='Enter Item Name' class="form-control" id="tags1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" autocomplete="off" style="display: none;" value="<?php echo $item_name[$i]; ?>" readonly />

												<input type="text" name="item_id[]" id="item_id" style="display: none;" value="<?php echo $item_id[$i]; ?>" readonly>

												<input type="text" name='itemName[]' placeholder='Enter Item Name' class="form-control" id="tags" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" autocomplete="off" style="display: block;" value="<?php echo $item_name[$i]; ?>" readonly/>
											...
											<td><input type="number" name='qty[]' placeholder='Enter Qty' class="form-control qty" step="0" min="0" onclick="calculateTheGst(this.value)" id="qty" value="<?php echo $item_qty[$i]; ?>" readonly/></td>

											<td><input type="number" name='price[]' placeholder='Enter Unit Price' class="form-control price" id="price" step="0.00" min="0"  value="<?php echo $item_price[$i]; ?>" readonly/></td>
											<td>
												<?php $tx = $d->get_tax_from_view($item_id[$i]); ?>
												<input type="number" name='taxrow[]' placeholder='Enter tax' class="form-control tax" id="tax" step="0.00" min="0"  value="<?php echo $tx->gst; ?>" readonly/>
											</td>
											<?php $rtotal=0; $rtotal = ((((int)$item_qty[$i])*((float)$item_price[$i]))/100 * ((float)($tx->gst))) ?>
											<td><input type="number" name='total[]' placeholder='0.00' class="form-control total" id="totalRow" readonly value="<?php echo (int)$item_rtotal[$i]; ?>" readonly /></td>
											<?php $tr = $tr + $rtotal; ?>
										</tr>
									<?php }?>
									</tbody>
								...
								<table class="table table-bordered table-hover" id="tab_logic_total">
									<tbody>
										<tr>
											...
											<td class="text-center"><input type="number" name='sub_total' placeholder='0.00' class="form-control" id="sub_total" readonly value="<?php echo (int)$s_all->item_sub_total; ?>" readonly/></td>
										</tr>
										...
										<tr >
											...
											<td class="text-center"><input type="number" name='' id="" placeholder='0.00' class="form-control" value="<?php echo $tr;?>" readonly/></td>
										</tr>
										<tr style="display: none;">
										...
										<tr>
											...
											<td class="text-center"><input type="number" name='total_amount' id="total_amount" placeholder='0.00' class="form-control" value="<?php echo $s_all->item_grand_total; ?>"readonly /></td>
										</tr>
									</tbody>
								</table>
...
<script>
	...
	// 		success: function(response1)
	// 		{
	// 			if(response1<qtyValue)
	// 			{
	// 				swal('Item is Less in Stock');
	// 				$('#qty').val(response1);
	// 			}
	// 		}
	// 	});
	...
	function addRowForBill()
	{
		...
		else
		{
			...
			$.ajax({
				url: "<?php echo URLROOT; ?>/pages/tempSaleData",
				type: "POST",
				data: {
					sta, sItem, sQty, sPrice, sTax, sTotal,item_id
				},
