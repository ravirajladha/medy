...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        <div class="col-md-8 col-lg-8">
            <h4 class="page-title">Distributor Orders</h4>
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col-6">
                            <h5 class="card-title mb-0">All Distributor Order</h5>
                        </div>
                        <div class="col-6">
...
<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true" id="processSaleModal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleLargeModalLabel">Distributor Order - <span id="pid"></span></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script type="text/javascript">
  $(document).ready(function()
    ...
    var inc = 0;
        $.ajax({
        type: "POST",
        url: '<?php echo URLROOT;?>/pages/getAllDistributor',
        data: {lim,off},
        cache: false,
        success:function(response)
        {
            $('#alldistributor').html(response);
        }
        });
        ...
        if($(window).scrollTop() + $(window).height() >= $(document).height())
        {
            ...
            { 
                inc++;
                $('#alldistributor').append(response);
            }
            });
        }
    });
...
<script>
    function processModal(id)
    {
        ...
        $('#pid').text(id);
        $('#buttonArea').html('<a href="<?php echo URLROOT ?>/pages/packageCreate_fordist/'+id+'"><button type="button" class="btn btn-info-rgba">Create Package</button></a><a href="<?php echo URLROOT ?>/pages/distributorInvoice/'+id+'"><button type="button" class="btn btn-info-rgba">view Invoice</button></a>');
        $.ajax({
            url: "<?php echo URLROOT; ?>/pages/processCreate_for_dist",
            type: "POST",
            data: {
                id
            },
        ...
        $.ajax({
            url: "<?php echo URLROOT; ?>/pages/createInvoice_for_distributor",
            type: "POST",
            data: {
                id
            },
