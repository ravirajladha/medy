...
<?php $st_details = $data['st_details'];
$st_order = $data['st_order']; ?>
<form method="post" action="<?php echo URLROOT;?>/pages/update_sales_order_for_stock_out">
	<input type="hidden" name="st_id" value="<?php echo $st_details->id; ?>">
				...
				<div class="card m-b-30">
					...
					<div class="card-body">
						<div class="form-group">
							...
							<select class="select2-single form-control" name="customer_id" id="cname" >
	                        	<option disabled>--Select--</option>
	                        	<?php foreach ($data['customer'] as $cu) { ?>

	                            <option <?php if(($st_details->customer_id)==($cu->id)){ ?> selected <?php }?> value="<?php echo $cu->id;?>"><?php echo $cu->customer_display_name;?></option>
							...
							<div class="col-md-12">
								<table class="table table-bordered table-hover" id="tab_logic">
									...
									<tbody id='addr1'>
										...
										<?php
											$sl_or = $data['st_order'];
											$item_id = explode("|||", $sl_or->item_id);	 
											$item_name = explode("|||", $sl_or->item_name);	 
											$item_qty = explode("|||", $sl_or->item_qty);	 
											$item_rec = explode("|||", $sl_or->item_rec);	 
											$item_price = explode("|||", $sl_or->item_price);	 
											$item_rtotal = explode("|||", $sl_or->item_rtotal);	 
										?>
										<?php for ($i=0; $i <sizeof($item_id); $i++) { 
											if($item_rec[$i]==1){ $r = "Box"; }else{ $r="Pieces"; }
										?>
										<?php $item_name[$i] = trim($item_name[$i]); ?>
										<tr>
											<td width="400"><input class="form-control"  value="<?php echo $item_name[$i]; ?>" readonly></td>
											<td></td>
											<td><input class="form-control " value="<?php echo $r ?>" readonly></td>
											<td><input class="form-control qty"  value="<?php echo $item_qty[$i]; ?>" ></td>
											<td><input class="form-control price"  value="<?php echo $item_price[$i]; ?>" readonly ></td>
											<td><input class="form-control total"  value="<?php echo $item_rtotal[$i]; ?>" readonly></td>
										</tr>
										<?php }?>
									</tbody>
								</table>
