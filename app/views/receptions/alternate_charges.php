      ...
      <form id="fed">
        <div class="table-responsive">
          ...
          <table class="table table-bordered table-hover" id="tab_logic">
            ...
            <?php if (isset($data['vis_id_det'])) { 
              $idArray = array();
              $qtyArray = array();
              foreach ($data['vis_id_det'] as $val)
              {
                $ipd_nursing_service = explode(',', $val->nursing_services);
              }
              for ($y=0; $y < sizeof($ipd_nursing_service); $y++)
                { 
                  if($ipd_nursing_service[$y] != null)
                  {
                    array_push($idArray, $ipd_nursing_service[$y]);
                  }
                }
              $afterRemove = array_unique($idArray);
              $afterRemove = array_values($afterRemove);
              
              for ($t=0; $t < sizeof($afterRemove); $t++) { 
              $tmp = array_count_values($idArray);
              $cnt = $tmp[$afterRemove[$t]];  
              ?>
              <tbody id="">
              <tr>
                ...
                {
                  $serViceName = $recObj->getServiceNameFull($idArray[$t]);
                ?>
                  <td>
                    <input id="searchbox" type="text" name='product[]' placeholder='Service ID / Name' class="form-control searchInput" autocomplete="off" value="<?php echo $serViceName->service_name;?>  | <?php echo $serViceName->service_id ?>" />
                ...
                ?>
                <td>
                  <input type="number" id="qtybox" name='qty[]' placeholder='Enter Qty' class="form-control qty" step="0" min="0" value="<?php echo $cnt ?>" />
                </td>
                <td>
                  <input type="number" id="cost" name='price[]' placeholder='Enter Unit Price' class="form-control price" step="0.00" min="0" value="<?php echo $serViceName->service_cost; ?>" />
                </td>
                <td>
    ...
    <div class="col-md-4">
      <table class="table table-bordered table-hover" id="tab_logic_total">
        <tbody>
          ...
          <tr>
            <th class="text-center mid_pad">Service ID</th>
            <td class="text-center">
    ...
    <div class="col-md-4">
      <table class="table table-bordered table-hover" id="tab_logic_total">
        <tbody>
          ...
          <tr>
            <th class="text-center" >Discount Authorized By</th>
            <td class="text-center">
                <input type="text" class="form-control" id="authorized_by" placeholder="Enter Name" name="authorized_by">
             </td>
          </tr>
    ...
    <div class="col-md-4">
      <table class="table table-bordered table-hover" id="tab_logic_total">
        <tbody>
          ...
          <tr>
            <th class="text-center mid_pad">Advance Payment</th>
            <td class="text-center">
                <input type="number" id="advance_pay" class="form-control" placeholder="0" name="advance_pay">
            </td>
          </tr>
          <!--<tr>-->
          <!--  <th class="text-center mid_pad">Balance</th>-->
          <!--  <td class="text-center">-->
          <!--      <input type="number" id="final_balance" class="form-control" placeholder="0" name="final_balance">-->
          <!--  </td>-->
          <!--</tr>-->
        </tbody>
      </table>
      ...
      <button style="margin-right: 10px;" id="sav" type="button" class="btn btn-success pull-right" onclick="xsave(<?php echo $val=1;?>);">Save</button>
  ...
  <script>
    $("#refresh").click(function(){
        ...
        if(confirm("Are you sure you want to cancel this?")){
            $('#fed').trigger("reset");
            window.location.href = "<?php echo URLROOT;?>/receptions/index";
        }
        else{
            return false;
        }
...
<script type='text/javascript'>
  function xsave(b)
  {
    ...
    var authorized_by = $('#authorized_by').val();
    ...
    $.ajax({
              url:'<?php echo URLROOT;?>/receptions/save_invoice',
              type:'POST',
              data:{inv_id_edit,status,service_id,invoice_bill,grand_total,patient_name,doctor_name,ipopid,pay_mode,amount_paid,dep_type,b,taxx,dist,advance_pay,authorized_by},
              success : function(data)
              {
                      ...
                      if(b==2)
                      {
                          swal({
                              ...
                              type: "success"
                               ...
                               window.location = "<?php echo URLROOT;?>/receptions/view_invoice/"+inv_id_edit+"";
                          });
