    ...
    <div class="col-sm-12">
        <div class="panel panel-default">
            <div class="panel-heading">
              ...
              <h3 class="panel-title">All Admissions</h3><br>
            </div>
...
<div class="modal fade" id="upDoc" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?php echo URLROOT; ?>/receptions/upload_file_from_admit" method="POST" enctype="multipart/form-data">
      <div class="modal-body">
		<div class="form-group">
        ...
        <div class="form-group">
			<label for="">Admit Id</label>
			<input type="number" id="admintIdForUpload" class="form-control" name="admitId" readonly>
		</div>
		<div class="form-group">
...
<div class="modal fade" id="discharge_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        ...
        <h5 class="modal-title" id="exampleModalLabel">Discharge Summary</h5>
      </div>
      	<form action="<?php echo URLROOT; ?>/receptions/discharge_summary_update" method="POST">
			<div class="modal-body">
				<div class="">
					<div class="form-group">
						<label for="">Discharge Type</label>
						<input type="hidden" name="admit_id" id="x_admit_id">
						<input type="hidden" name="patient_id" id="x_patient_id">
			...
			<div class="modal-footer">
				<button type="submit" class="btn btn-primary">Discharge Summary</button>
			</div>
      	</form>
    </div>
  </div>
...
<script type="text/javascript">
  $(document).ready(function()
	...
	var inc = 0;
		$.ajax({
		type: "POST",
		url: '<?php echo URLROOT;?>/receptions/all_admissions1',
		data: {lim,off},
		cache: false,
  ...
  $(document).ready(function(){
        $("#admit_id_search").keyup(function(){
          ...
          if(admit_id != 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/receptions/search_by_admit_id',
              data:{admit_id},
              success : function(data)
              {
  ...
  $(document).ready(function(){
        $("#admit_pid_search").keyup(function(){
          ...
          if(admit_id != 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/receptions/search_by_p_id',
              data:{admit_id},
              success : function(data)
              {
                $('#all_admissions_display').html(data);
  ...
  $(document).ready(function(){
        $("#admit_pname_search").keyup(function(){
          ...
          if(admit_id != 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/receptions/search_by_p_name',
              data:{admit_id},
              success : function(data)
              {
                $('#all_admissions_display').html(data);
  ...
  $(document).ready(function(){
        $("#admit_dname_search").keyup(function(){
          ...
          if(admit_id != 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/receptions/search_by_d_name',
              data:{admit_id},
              success : function(data)
              {
                $('#all_admissions_display').html(data);
  ...
  $(document).ready(function(){
        $("#get_wrt_date").click(function(){
          ...
          if(start != '' && end != '')
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/receptions/get_all_admissions_wrt_dates',
              data:{start, end},
              success : function(data)
              {
                $('#all_admissions_display').html(data);
...
<script type="text/javascript">
  ...
  function uploadDoc(admitId)
  {
	  $('#admintIdForUpload').val(admitId);
	  $('#upDoc').modal('show');
  }
</script>

<script>
	function open_ds(admit_id, patient_id)
	{
		$('#x_admit_id').val(admit_id);
		$('#x_patient_id').val(patient_id);
