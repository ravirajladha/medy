    ...
    <div class="col-sm-12">
        <div class="panel panel-default">
            <div class="panel-heading">
              <a href="<?php echo URLROOT; ?>/receptions/newAppointment" style="float: right;"><button class="btn btn-info btn-xs m-b-5 ml-2">New Appointment</button></a>
              <a href="<?php echo URLROOT; ?>/receptions/donwloadExcel_appointments" style="float: right;margin-right:5px;"><button class="btn btn-info btn-xs m-b-5 mr-2 pr-2">Download Excel</button></a>
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <input type="text" id="admit_dname_search" name="example-input2-group2" class="form-control" placeholder="Search By Doctor Name" style="margin-top: 10px;">
                </div>
             </div>
             <br>
             <div class="row">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#admit_dname_search").keyup(function(){
            ...
            var patient_name = $('#admit_dname_search').val();
                if(patient_name!=0)
                {
                    $.ajax({
                      type:'POST',
                      url:'<?php echo URLROOT;?>/receptions/search_by_dnameForAppointments_value',
                      data:{patient_name},
                      success : function(data)
                      {
                            $('#allAppointmentsDisplay').html(data);
                      }
                    });
                }
                else
                {
                    $(document).ready(function()
                    {
                        ...
                        var inc = 0;
                          $.ajax({
                            type: "POST",
                            url: '<?php echo URLROOT;?>/receptions/allAppointments',
                            data: {lim,off},
                            cache: false,
