  ...
  <div class="panel panel-default">
      <form id="fed">
        ...
        <div class="form-group row col-md-4">
            <label for="">Select the Lab</label>
            <select name="" id="refLabId" class="form-control">
              <?php foreach ($data['ref'] as $ref) {
               ?>
              <option value="<?php echo $ref->ref_lab_id; ?>"><?php echo $ref->ref_lab_name; ?></option>
              <?php } ?>
            </select>
          </div>
...
<script type='text/javascript'>
  function xsave(b)
  {
    ...
    var service_id = '';
    var refId = $('#refLabId').children('option:selected').val();
    ...
    $.ajax({
              url:'<?php echo URLROOT;?>/receptions/save_invoice1',
              type:'POST',
              data:{inv_id_edit,status,service_id,invoice_bill,grand_total,patient_name,doctor_name,ipopid,pay_mode,amount_paid,dep_type,b,taxx,dist,refId},
              success : function(data)
              {
                if(data == "error")
                {
