...
<div class="modal fade" id="upDoc" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?php echo URLROOT; ?>/admin/upload_file_from_visit" method="POST" enctype="multipart/form-data">
  ...
  $(document).ready(function()
          {
            ...
            var inc = 0;
              $.ajax({
                type: "POST",
                url: '<?php echo URLROOT;?>/admin/all_visit1',
                data: {lim,off},
                cache: false,
  ...
  $(document).ready(function(){
        $("#visit_id_search").keyup(function(){
          ...
          if(visit_id != 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/admin/search_by_visit_id',
              data:{visit_id},
  ...
  $(document).ready(function(){
        $("#patient_id_search").keyup(function(){
          ...
          if(pat_id!= 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/admin/search_by_patient_id',
              data:{pat_id},
              success : function(data)
              {
                $('#all_visit_display').html(data);
  ...
  $(document).ready(function(){
        $("#pat_name").keyup(function(){
          ...
          if(pat_name!= 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/admin/search_by_pat_name',
              data:{pat_name},
              success : function(data)
              {
                $('#all_visit_display').html(data);
  ...
  $(document).ready(function(){
        $("#doc_name").keyup(function(){
          ...
          if(pat_name!= 0)
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/admin/search_by_doc_name',
              data:{pat_name},
              success : function(data)
              {
                $('#all_visit_display').html(data);
...
<script type="text/javascript">
  function updateFinish(id)
  {
      swal({   
            ...
            if (isConfirm) {     
                window.location.replace('<?php echo URLROOT; ?>/admin/finishVisit/'+id+'');  
            } else {     
  ...
  $(document).ready(function(){
        $("#get_wrt_date").click(function(){
          ...
          if(start != '' && end != '')
          {
            $.ajax({
              type:'POST',
              url:'<?php echo URLROOT;?>/admin/get_all_visit_wrt_dates',
              data:{start, end},
              success : function(data)
              {
                $('#all_visit_display').html(data);
